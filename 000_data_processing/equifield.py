...
for time in ['060', '120', '180', '240', '300']:
    ...
    for i, try_name in enumerate(try_names):
        ...
        data_file = data_path_equifield + "equifield" + "t=00" + time
        if binfile:
        ...
        #mlab.pipeline.iso_surface(src, contours=[s.max()*0.8, ], opacity=0.3)
        if plot_isosurfs:
            mlab_contour(data_file, equifield_data, M)
    ...
    plt.xlabel("dT [K]")
    plt.savefig(pf.data_dir([exp_name, res_dir]) + "equi_t=" + time + "_hist.png")   
    plt.close()


#%%
def mlab_contour(data_path, data, M):
    s = data
    
    mlab.figure(size=(1200,1200))
    
    src = mlab.pipeline.scalar_field(M[0], M[1], M[2], s)
   # mlab.pipeline.iso_surface(src, contours=[s.min()*0.2, ], opacity=0.)
    mlab.axes()
